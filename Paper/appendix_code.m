...
quiz = quiz + set(X > 0);
for i = 1:nu,
    quiz = quiz + set([-X, X, (A{i}*X-B{i}*Y)',
             gamma{i,1}*(AN{i,1}*X-BN{i,1}*Y)',
             gamma{i,2}*(AN{i,2}*X-BN{i,2}*Y)',
             gamma{i,3}*(AN{i,3}*X-BN{i,3}*Y)',
             gamma{i,4}*(AN{i,4}*X-BN{i,4}*Y)';
             X, -inv(Q{i}), zeros(n,n), zeros(n,n), zeros(n,n),
             zeros(n,n), zeros(n,n);
             (A{i}*X-B{i}*Y), zeros(n,n),-X, zeros(n,n), zeros(n,n),
             zeros(n,n), zeros(n,n);
             gamma{i,1}*(AN{i,1}*X-BN{i,1}*Y), zeros(n,n),
             zeros(n,n),-X, zeros(n,n), zeros(n,n), zeros(n,n);
             gamma{i,2}*(AN{i,2}*X-BN{i,2}*Y), zeros(n,n),
             zeros(n,n), zeros(n,n), -X, zeros(n,n), zeros(n,n);
             gamma{i,3}*(AN{i,3}*X-BN{i,3}*Y),zeros(n,n), zeros(n,n),
             zeros(n,n), zeros(n,n), -X, zeros(n,n);
             gamma{i,4}*(AN{i,4}*X-BN{i,4}*Y),zeros(n,n), zeros(n,n),
             zeros(n,n), zeros(n,n), zeros(n,n), -X]<0);
end;
...
G = G + set(Pn > 0);
    for i = 1:nu,
    G = G + set((A{i}-B{i}*K )'*Pn *(A{i}-B{i}*K )
        + sump{i}+Q{i}+K'*R{i}*K-Pn <0);
end;
